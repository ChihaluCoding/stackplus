...
final class StackPlusDurabilityWarningScreen extends Screen {
    ...
    private CheckboxWidget suppressWarning;
    ...
    protected void init() {
        ...
        suppressWarning = CheckboxWidget.builder(Text.translatable("screen.stackplus.durability_warning.suppress"), this.textRenderer)
                .pos(centerX - 120, buttonY - 28)
                .checked(false)
                .callback((checkbox, selected) -> suppressWarningSelected = selected)
                .build();
        suppressWarning.setTooltip(Tooltip.of(Text.translatable("tooltip.stackplus.durability_warning.suppress")));
        addDrawableChild(suppressWarning);
